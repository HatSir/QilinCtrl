...
# 曲线
strokeWidth = 1.3622   # 线宽为strokeWidth * 10 mil
clearance = 4.35     #两条线中心间距为clearance* 10 mil  线边缘的间距为（clearance - strokeWidth）*10mil
order = 6   #曲线阶数
...
def PCB_hilbert():
    ...
    print("TRACK~" + str(strokeWidth) + "~1~BED~", end='', file=f)

    for i in range(0, len(x) - 1):
        # print >> f, str(i)+'\n'
        print(str(x[i]*clearance + offsetx) + ' ' +
              str(y[i]*clearance + offsety) + ' ', end='', file=f)
    print(str(x[len(x) - 1]*clearance + offsetx) + ' ' + str(y[len(x) - 1]
          * clearance + offsety) + "~gee123~0", end='', file=f)
